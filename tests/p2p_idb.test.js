...
describe('P2PService - IndexedDB Fallback', () => {
    ...
    test('stores chunks in IDB when File System Access API is missing', async () => {
        ...
        // 1. Send Meta Packet
        const metaData = { type: 'meta', name: fileName, size: fileSize, mime, totalChunks };
        await p2p._processChunk(metaData);

        expect(db.deleteFileChunks).toHaveBeenCalledWith(fileName);
        expect(p2p.receivingChunks.has(fileName)).toBe(true);
        expect(p2p.receivingChunks.get(fileName).storage).toBeUndefined(); // Or 'idb' if I set it explicitly, but currently I don't set a flag, I just rely on logic flow.

        // 2. Send Chunk 0
        const chunk0 = { type: 'chunk', index: 0, total: totalChunks, data: new ArrayBuffer(500), name: fileName };
        await p2p._processChunk(chunk0);

        expect(db.addChunk).toHaveBeenCalledWith(fileName, 0, chunk0.data);

        // 3. Send Chunk 1 (Completion)
        const chunk1 = { type: 'chunk', index: 1, total: totalChunks, data: new ArrayBuffer(500), name: fileName };
        ...
        expect(db.addChunk).toHaveBeenCalledWith(fileName, 1, chunk1.data);
        expect(db.getFileChunks).toHaveBeenCalledWith(fileName);
